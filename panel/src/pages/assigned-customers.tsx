...
export function AssignedCustomersPage() {
  ...
  const customersQuery = useQuery({
    queryKey: ['assigned-customers', apiFilters],
  ...
  const tabsQuery = useQuery({
    queryKey: ['assigned-customers', 'tabs'],
    enabled: user?.role === 'agent',
    queryFn: async () => {
  ...
  >({
    mutationFn: async ({ customerId, status, reason, meta }) => {
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['assigned-customers', 'tabs'] })
