...
export function AssignedCustomersPage() {
  ...
  const query = useQuery({
    queryKey: ['assigned-customers', user?.id, { page: filters.page, search: debouncedSearch }],
    enabled: Boolean(user?.id && user.role === 'agent'),
    queryFn: async () => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="text-2xl font-bold tracking-tight">مشتریان تخصیص‌یافته</h1>
          <p className="text-muted-foreground">مشتریانی که شما مسئول آن‌ها هستید</p>
        </div>
      ...
      {query.isLoading ? (
        ...
        <Card>
          <CardContent className="p-12 text-center text-muted-foreground">
            <p>هنوز مشتری تخصیص‌یافته‌ای ندارید.</p>
          </CardContent>
        </Card>
      ) : (
    ...
    <Card>
      <CardContent className="space-y-3 p-6">
        ...
        <div className="grid gap-2 text-xs text-muted-foreground sm:grid-cols-2">
          ...
          <span>سرپرست: {customer.assigned_supervisor_name || '-'}</span>
